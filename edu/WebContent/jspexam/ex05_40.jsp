...
<body>
	<c:choose>
		...
		<form method="POST" action="/edu/jspexam/ex05_40.jsp">
		...
		<c:otherwise>
			...
			<c:catch var="ex">
				...
				url="http://ws.bus.go.kr/api/rest/busRouteInfo/getBusRouteList?serviceKey=%2BjzsSyNtwmcqxUsGnflvs3rW2oceFvhHR8AFkM3ao%2Fw50hwHXgGyPVutXw04uAXvrkoWgkoScvvhlH7jgD4%2FRQ%3D%3D&strSrch=${param.num }"/>
				...
				<x:if select="$xdata//headerCd=0">
					...
					<x:forEach select="$xdata//itemList">
						<x:if select="busRouteNm = $busnum">
							버스번호 : <x:out select ="stStationNm"/><br>
			...
			<hr>
			<a href="/edu/jspexam/ex05_40.jsp">입력 화면으로</a>
		</c:otherwise>
	</c:choose>
</body>
</html>
