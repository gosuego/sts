...
<c:catch var="ex">
	...
	<x:if select="$xdata//학생[@번호='st0004]">
		번호가 st0004 번인 학생 이름 : <x:out
			select="$xdata//학생[@번호='st0004']/이름"></x:out><br>
			
	</x:if>
	<x:choose>
		<x:when select="$xdata//학생[@번호='st0005']">
			번호가 st0005 번인 학생 이름 <x:out select="$xdata//학생[@번호='st0005']/이름"/>
		</x:when>
		<x:otherwise>
			<h3>번호가 st0005 번인 학생 데이타가 존재하지 않습니다!!</h3>
		</x:otherwise>
	</x:choose>
	<c:if test="${ex !=null }">
		<h3>${ex }</h3>
